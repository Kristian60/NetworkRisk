...
def WildBootstrap(data, H):
    ...
    for i, method in enumerate(methods):
        ...
        if method == 'regular':
            ...
            shockMatrix = np.array(
                [random.choice(resid.values) * np.random.normal() for x in range(len(simReturns) + 15)])
        elif method == 'mammen':
            shockMatrix = np.array([random.choice(resid.values) * (-(np.sqrt(5) - 1) / 2) if np.random.rand() < (
                                                                                                                np.sqrt(
                                                                                                                    5) - 1) / (
                                                                                                                2 * np.sqrt(
                                                                                                                    5)) else random.choice(
                resid.values) * ((np.sqrt(5) + 1) / 2) for x in range(len(simReturns) + 15)])
        elif method == 'rademacher':
            shockMatrix = np.array(
                [random.choice(resid.values) if np.random.rand() < 0.5 else random.choice(resid.values) * (-1) for x in
                 range(len(simReturns) + 15)])
        ...
        print method, '   ', np.mean(simReturns[0]), np.std(simReturns[0]), scipy.stats.skew(
            simReturns[0]), scipy.stats.kurtosis(simReturns[0])
