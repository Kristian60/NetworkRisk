...
flist = sorted(os.listdir('Z:/TAQ/TAQHDF5/'),reverse=True)
slist = os.listdir('E:/Speciale Data/')

for ff in flist[::-1]:
    print ff
    if ff.replace('taq_', '')[:4] >= '2003' and ff.replace('taq_', '')[:4] < '2009' and str(ff).replace('taq_','').replace('h5','csv') not in os.listdir('E:/Speciale Data/'):
        pd.DataFrame().to_csv('E:/Speciale Data/' + ff.replace('taq_', '').replace('.h5', '') + '.csv')
        ...
        df.to_csv('E:/Speciale Data/' + ff.replace('taq_', '').replace('.h5', '') + '.csv', columns=['time', 'price', 'sym'],
                          index=False)
